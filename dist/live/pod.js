...
import { assertWithinPodScope, normalizePodBase, podScopedUrl, SsrfError, } from "@jeswr/guarded-fetch";
...
export class LivePod {
     ...
     */
    async put(url, body, contentType, options = {}) {
        ...
        if (response.status === 412) {
            throw new LivePodError(`conditional PUT failed (412) for ${target} — the resource already exists ` +
                "(create-only) or its ETag moved (lost update)", { status: 412 });
        }
        if (!response.ok) {
     ...
     */
    async ensureContainer(url) {
        const target = assertWithinPodScope(this.base, url, { allowRoot: true });
        if (!target.endsWith("/")) {
            throw new LivePodError(`ensureContainer target must be a container URL (trailing /): ${target}`);
        }
    ...
    assertNoRedirect(response, method, target) {
        if (isRedirect(response)) {
            throw new SsrfError(`refused a redirect on a credentialed ${method} to ${target} ` +
 ...
 */
export function ancestorContainers(base, target) {
 ...
 */
export function parentContainer(url) {
 ...
 */
export function parseAclLink(linkHeader) {

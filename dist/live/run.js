...
import { decodeUpgradeOffer, decodeUpgradeResponse, encodeUpgradeOffer, encodeUpgradeResponse, mayDowngradeToNl, parseIntent, validateIntent, verifyProtocolDocument, } from "@jeswr/solid-a2a";
...
import { buildAclDocument, ownerRule } from "./acl.js";
import { VALID_FROM, VALID_UNTIL } from "./cast.js";
import { createDiscoveryFetch } from "./fetch.js";
import { buildEnvelope, dereferenceAnnouncedObject, discoverInbox, postNotification, readInbox, } from "./ldn.js";
...
async function statusOfGet(pod, url) {
    try {
 ...
 */
export async function runLiveScenario(substrate, options = {}) {
    ...
    const mandate = buildLiveMandate(cast);
    const agreement = buildLiveAgreement(cast);
    const instituteInternal = buildLiveInstituteInternal(cast);
    ...
    if (discovery.verification?.valid !== true) {
        throw new LiveScenarioRefusal(`discovery failed: ${JSON.stringify(discovery.verification?.issues ?? [])}`);
    }
    ...
    const nl = `share read access to ${cast.alice.records} with ${cast.institute.webId} ` +
    ...
    const request = requestContextFromA2AIntent({ action: "read", target: cast.alice.records }, { purpose: cast.purpose, dateTime: now.toISOString() });
    const primaryChain = {
    ...
    if (!verification.authorized) {
        throw new LiveScenarioRefusal(`authorization denied (${verification.code}): ${verification.reason}`);
    }
    ...
    if (before !== 403) {
        throw new LiveScenarioRefusal(`expected agent R's pre-grant read of the records to be 403, got ${String(before)}`);
    }
    ...
    if (after !== "ok") {
        throw new LiveScenarioRefusal(`expected agent R's post-grant read of the records to be 200, got ${String(after)}`);
    }
    ...
    const internalDocUrl = cast.instituteInternalId.split("#")[0];
    ...
    await publishDerivedSummary(institutePod, cast.institute.webId, cast.derivedArtifact, activityIri);
    ...
    if (announceNote?.objectIri !== undefined) {
        ...
        if (bundle !== undefined) {
            ...
            await aliceCopyPod.put(`${cast.alice.engagementBase}activities/${activityId}.ttl`, bundle.body, bundle.contentType || "text/turtle");
        }
    }
    return {
        now,
...
async function writeResourceAcl(ownerPod, resource, rules) {
...
async function publishDerivedSummary(institutePod, ownerWebId, artifact, activityIri) {

 ...
 */
function actorFetch(state, creds) {
    const bound = async (input, init) => {
...
export async function createActorSessions(actors) {
    const entries = Object.entries(actors);
    const built = await Promise.all(entries.map(async ([label, actor]) => [label, await createActorSession(actor)]));
 ...
 */
export function createInteractiveActorSession() {
    throw new Error("interactive auth-code session is not implemented — reserved for the " +

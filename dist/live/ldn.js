...
export const KNOWN_LDN_TYPES = ["Offer", "Accept", "Reject", "Announce"];
const KNOWN_TYPE_SET = new Set(KNOWN_LDN_TYPES);
...
function hasAs2Context(context) {
    ...
    if (context !== null && typeof context === "object") {
        ...
        return Object.values(context).some((v) => v === AS2_CONTEXT);
    }
    return false;
}
...
function assertNoRedirect(response, what) {
 ...
 */
export async function discoverInbox(fetch, webId) {
    const docUrl = (() => {
    ...
    if (inboxUrl.origin !== webIdOrigin) {
        throw new LdnError(`refusing an out-of-origin ldp:inbox ${inbox} for WebID ${webId} (origin ${webIdOrigin})`);
    }
 ...
 */
export async function postNotification(options) {
 ...
 */
export function parseNotification(id, body, allowedObjectOrigins) {
    let parsed;
    try {
 ...
 * order-stable.
 */
export async function readInbox(options) {
 ...
 */
export async function dereferenceAnnouncedObject(guardedFetch, objectIri, allowedObjectOrigins) {
    let origin;
    try {

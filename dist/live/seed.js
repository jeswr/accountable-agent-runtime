...
import { buildBitstringStatusListCredential, generateKeyPairForSuite, issue, publishVerificationMethod, } from "@jeswr/solid-vc";
import { GraphBuilder, parseTurtle, serializeTurtle } from "../rdf.js";
import { buildRuntimeProtocolDocument } from "../scenario/protocol.js";
import { seedAccount } from "./account.js";
import { buildAclDocument, ownerRule } from "./acl.js";
import { createActorSessions } from "./auth.js";
import { actorBasesFor, buildCast, VALID_FROM, VALID_UNTIL, } from "./cast.js";
import { bootCss } from "./css.js";
...
async function profileTurtle(actor, name, serverBase, key, extra) {
...
async function seedProfile(pod, actor, name, serverBase, key, extra) {
...
async function writeAcl(ownerPod, resource, rules) {
...
function delegateRule(name, agents, container, modes) {
...
export async function seedDemo(options = {}) {
    ...
    try {
        ...
        // --- sessions ---------------------------------------------------------
        const sessions = await createActorSessions({
        ...
        const anpQuads = [
            ...(await parseTurtle(await agentDescription.toTurtle(), "text/turtle", cast.agentR.profileDoc)),
        ];
        ...
        await seedProfile(institutePod, cast.institute, "Institute", serverBase, actorKeys.institute, pointerQuads);
        await seedProfile(agentRPod, cast.agentR, "Institute research agent", serverBase, actorKeys.agentR, anpQuads);

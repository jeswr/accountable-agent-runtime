...
export async function runScenario(options = {}) {
    ...
    if (discovery.verification?.valid !== true) {
        throw new ScenarioRefusal(`discovery failed: ${JSON.stringify(discovery.verification?.issues ?? [])}`);
    }
    ...
    const response = decodeUpgradeResponse(encodeUpgradeResponse({ protocolHash: offer.protocolHash, accept: true }));
    ...
    const nl = `share read access to ${CAST.records} with ${CAST.inst} ` +
    ...
    const request = requestContextFromA2AIntent({ action: "read", target: CAST.records }, { purpose: CAST.purpose, dateTime: now.toISOString() });
    const primaryChain = {
    ...
    if (!verification.authorized) {
        throw new ScenarioRefusal(`authorization denied (${verification.code}): ${verification.reason}`);
    }
    ...
    pod.put(`${CAST.engagementBase}institute-internal.ttl`, await policyToTurtle(instituteInternal), "text/turtle");
    ...
    pod.put(`${CAST.aliceInbox}${activityId}.ttl`, await serializeTurtle(activityQuads), "text/turtle");

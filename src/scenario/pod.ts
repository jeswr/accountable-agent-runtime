...
export class InMemoryPod implements ResourceSink, ResourceSource {
   ...
   */
  fetch: typeof globalThis.fetch = ((input: RequestInfo | URL): Promise<Response> => {
    const raw =
    ...
    if (resource === undefined) {
      return Promise.resolve(
        new Response(`Not found: ${url}`, { status: 404, statusText: "Not Found" }),
      );
    }
    return Promise.resolve(

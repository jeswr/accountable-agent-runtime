...
import { type PresentedChain, verifyAgentAuthority } from "@jeswr/agent-authz-verifier";
import {
...
function chainsOf(
  runResult: LiveRunResult,
  policies: { mandate: OdrlPolicy; agreement: OdrlPolicy; institute: OdrlPolicy },
  contents: { mandate: string; agreement: string; institute: string },
  overrides: { agreementVc?: VerifiableCredential } = {},
): { primary: PresentedChain; actor: PresentedChain } {
  const cast = runResult.cast;
  const primary: PresentedChain = {
    credentials: [
 ...
 */
export async function runNegativeActs(
  substrate: LiveSubstrate,
  runResult: LiveRunResult,
): Promise<NegativeActsResult> {
  ...
  const happyRequest = requestContextFromA2AIntent(
  ...
  const forgedAgreementVc = await issueAgentAuthorization(
    ...
    forgeKey, // NOT agent A's published key → the resolver's real key refuses the signature
  );
  ...
  const omitReport: AuditReport = await auditArtifact(trace, omittedArtifact, {
    resolveKey: keyResolver.resolveKey,
    isControlledBy: keyResolver.isControlledBy,
    resolveStatus: liveStatusResolver(auditFetch, { now }),
  });
...
async function runRevokedSubtree(
  substrate: LiveSubstrate,
  runResult: LiveRunResult,
  chains: {
    primary: PresentedChain;
    actor: PresentedChain;
    request: ReturnType<typeof requestContextFromA2AIntent>;
  },
): Promise<NegativeActsResult["revokedSubtree"]> {
  ...
  const listVc = JSON.parse(original.body) as VerifiableCredential;
  ...
  let reRun: Awaited<ReturnType<typeof verifyAgentAuthority>>;
  let reverted = false;
  try {

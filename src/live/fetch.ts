 ...
 */
export function assertBaseTransport(base: string): { readonly loopback: boolean } {
  let url: URL;
  try {
    url = new URL(base);
  } catch {
  ...
  if (url.protocol === "http:" && !loopback) {
    throw new SsrfError(
      `refusing plaintext http: to a non-loopback host (${url.hostname}); ` +
 ...
 */
export function createDiscoveryFetch(
  base: string,
